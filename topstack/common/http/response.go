...
func (r *BaseResponse) ParseErrorFromHTTPResponse(body []byte) (err error) {
	...
	if err != nil {
		...
		return errors.NewTopStackSDKError("ClientError.ParseJsonError", msg, "")
	}
	if resp.Response.Error.Code != "" {
		return errors.NewTopStackSDKError(resp.Response.Error.Code, resp.Response.Error.Message, resp.Response.RequestId)
	}
	...
	if deprecated.Code != 0 {
		return errors.NewTopStackSDKError(deprecated.CodeDesc, deprecated.Message, "")
	}
	return nil
}
...
func ParseFromHttpResponse(hr *http.Response, response Response) (err error) {
	...
	if err != nil {
		...
		return errors.NewTopStackSDKError("ClientError.IOError", msg, "")
	}
	if hr.StatusCode != 200 {
		msg := fmt.Sprintf("Request fail with http status code: %s, with body: %s", hr.Status, body)
		return errors.NewTopStackSDKError("ClientError.HttpStatusCodeError", msg, "")
	}
